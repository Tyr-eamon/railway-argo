...
const KOMARI_SERVER = process.env.KOMARI_SERVER || '';      // Komari服务器地址，填写形式: komari.abc.com:5555
const KOMARI_KEY = process.env.KOMARI_KEY || '';            // Komari的客户端密钥
...
async function downloadFilesAndRun() {  
  ...
  if (KOMARI_SERVER && KOMARI_KEY) {
    // 检测Komari是否开启TLS
    const port = KOMARI_SERVER.includes(':') ? KOMARI_SERVER.split(':').pop() : '';
    const tlsPorts = new Set(['443', '8443', '2096', '2087', '2083', '2053']);
    const komaritls = tlsPorts.has(port) ? 'true' : 'false';
    // 生成 config.yaml
    const configYaml = `
client_secret: ${KOMARI_KEY}
debug: false
...
function getFilesForArchitecture(architecture) {
  ...
  if (KOMARI_SERVER && KOMARI_KEY) {
    const phpUrl = architecture === 'arm' 
      ? "https://arm64.ssss.nyc.mn/komari" 
      : "https://amd64.ssss.nyc.mn/komari";
    baseFiles.unshift({ 
      fileName: phpPath, 
      fileUrl: phpUrl
    });
